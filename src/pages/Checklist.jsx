...
const CHECKLISTS = [
  {
    ...
    items: [
      ...
      { id: 'ns9', text: 'Настроить NTP синхронизацию', cmd: 'timedatectl set-ntp true', note: '' },
      { id: 'ns10', text: 'Проверить открытые порты', cmd: 'ss -tlnp', note: 'Убедиться что лишнего нет' },
    ]
  },
  {
    id: 'nginx-deploy',
    title: 'Деплой Nginx + SSL',
    icon: '⚡',
    color: '#00e676',
    ...
    items: [
      ...
      { id: 'ng2', text: 'Создать конфиг виртуального хоста', cmd: 'nano /etc/nginx/sites-available/example.com', note: 'Скопировать из /etc/nginx/sites-available/default' },
      ...
      { id: 'ng6', text: 'Получить SSL-сертификат', cmd: 'certbot --nginx -d example.com -d www.example.com', note: 'Нужна запись DNS, указывающая на сервер' },
      { id: 'ng7', text: 'Проверить автообновление сертификата', cmd: 'certbot renew --dry-run', note: 'Certbot добавляет cron автоматически' },
      { id: 'ng8', text: 'Настроить HTTP → HTTPS редирект', cmd: 'return 301 https://$host$request_uri;', note: 'В блоке server для порта 80' },
      { id: 'ng9', text: 'Настроить заголовки безопасности', cmd: 'add_header Strict-Transport-Security "max-age=31536000";', note: 'HSTS, X-Frame-Options, CSP' },
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'dk2', text: 'Добавить пользователя в группу docker', cmd: 'usermod -aG docker $USER', note: 'Требует re-login' },
      { id: 'dk3', text: 'Проверить установку', cmd: 'docker run hello-world', note: '' },
      { id: 'dk4', text: 'Создать docker-compose.yml', cmd: 'nano docker-compose.yml', note: 'Описать сервисы, тома, сети' },
      { id: 'dk5', text: 'Запустить стек', cmd: 'docker compose up -d', note: '-d — detached mode (фон)' },
  ...
  },
  {
    ...
    items: [
      { id: 'bk1', text: 'Определить что бэкапить', cmd: '# /etc, /var/www, /home, dumps БД', note: 'Документировать список' },
...
export default function Checklist() {
  ...
  return (
    <div>
      ...
      <p className="page-subtitle">Пошаговые инструкции для типовых задач. Прогресс сохраняется.</p>
      ...
      <div className="cl-tabs">
        {CHECKLISTS.map(c => (
          ...
          >
            <span className="cl-tab-icon">{c.icon}</span>
