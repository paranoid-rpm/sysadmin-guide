...
const ARTICLES = [
  {
    ...
    content: [
      ...
      { h: 'Уровни снизу вверх', table: [
        { num: '1', name: 'Физический', en: 'Physical', desc: 'Биты по кабелю. Стандарты: Ethernet, USB, Wi‑Fi (физический уровень).' },
      ...
      { h: 'Практическое применение', p: 'При диагностике сети удобно двигаться снизу вверх: сначала проверить физическое соединение (L1), затем ARP (L2), пинг (L3), порт (L4) и только потом приложение (L7). Это экономит время.' },
    ]
  },
  {
    id: 'tcp-handshake',
    cat: 'Сети',
    title: 'TCP: трёхстороннее рукопожатие',
    icon: '',
    color: '#00aaff',
    ...
    content: [
      ...
      { h: 'Закрытие соединения', p: 'Закрытие — четырёхшаговый процесс (Four‑Way Handshake): FIN → ACK → FIN → ACK. Каждая сторона закрывает своё направление независимо. Состояние TIME_WAIT длится 2×MSL (~60 секунд) для перехвата запоздавших пакетов.' },
      { h: 'Диагностика', p: 'ss -tan показывает состояния соединений: ESTABLISHED, TIME_WAIT, CLOSE_WAIT. Большое количество TIME_WAIT — нормально при высоком трафике. CLOSE_WAIT — признак утечки: приложение не закрывает соединения.' },
    ]
  },
  {
    id: 'dns',
    ...
    content: [
      { h: 'Иерархия DNS', p: 'DNS — распределённая иерархическая база данных. Корень (.) → TLD (.ru, .com) → домен второго уровня (example.com) → поддомен (www.example.com). Каждый уровень обслуживается своими серверами.' },
      { h: 'Путь запроса', list: [
  ...
  },
  {
    ...
    icon: '',
    color: '#ffb300',
    short: 'От нажатия кнопки до приглашения login: BIOS → bootloader → kernel → init.',
    content: [
      { h: 'Этапы загрузки', list: [
        'POST: BIOS/UEFI проверяет оборудование.',
        'Bootloader: GRUB2 загружается из MBR/ESP и показывает меню.',
      ...
      { h: 'Systemd targets', p: 'Targets заменили runlevels из SysV init. rescue.target (~runlevel 1) — однопользовательский режим для восстановления. multi-user.target (~runlevel 3) — полноценная система без GUI. graphical.target (~runlevel 5) — с графическим интерфейсом.' },
      { h: 'Диагностика загрузки', p: 'systemd-analyze blame — какие юниты замедляют загрузку. systemd-analyze plot > boot.svg — SVG‑диаграмма. journalctl -b -1 — логи предыдущей загрузки (полезно после сбоя).' },
    ]
  },
  {
    id: 'processes',
    ...
    content: [
      { h: 'Состояния процесса', table: [
        { num: 'R', name: 'Running',   en: 'Выполняется',  desc: 'Процесс занимает CPU прямо сейчас.' },
        { num: 'S', name: 'Sleeping',  en: 'Ждёт',         desc: 'Interruptible sleep: ждёт I/O или события.' },
        { num: 'D', name: 'Disk Wait', en: 'D‑состояние',  desc: 'Uninterruptible: ждёт I/O. SIGKILL не поможет.' },
      ...
      { h: 'Основные сигналы', list: [
        'SIGTERM (15) — мягкое завершение. Процесс может перехватить сигнал и корректно завершиться.',
        'SIGKILL (9) — принудительное уничтожение. Не перехватывается.',
        'SIGHUP (1) — перечитать конфиг (например, nginx -s reload).',
        'SIGSTOP (19) — приостановить. SIGCONT (18) — продолжить.',
      ]},
      { h: 'Управление ресурсами', p: 'nice (-20..+19) — приоритет CPU. renice -n 10 -p PID — снизить приоритет. ulimit ограничивает ресурсы сессии. cgroups (через systemd) — лимиты CPU, RAM и I/O для групп процессов.' },
    ]
  },
  {
    id: 'ssl-tls',
    cat: 'Безопасность',
    title: 'TLS: как работает handshake',
    icon: '',
    color: '#a78bfa',
    ...
    content: [
      { h: 'TLS 1.3 Handshake', list: [
        ...
        'ServerHello и Certificate: сервер выбирает алгоритм и отправляет сертификат.',
        'Клиент проверяет сертификат по цепочке доверия до корневого CA.',
        'Обе стороны вычисляют общий ключ (ECDHE). Далее всё шифруется симметрично.',
        'Finished: стороны подтверждают корректность handshake, после чего идут данные.',
      ]},
