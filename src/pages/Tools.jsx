...
const TOOLS = [
  ...
  { id: 2, name: 'Grafana', cat: 'monitoring', icon: '📊', color: '#f46800',
    desc: 'Визуализация метрик, логов и трейсов. Поддерживает 50+ источников данных.',
    tags: ['дашборды', 'визуализация', 'алерты'],
    install: 'docker run -p 3000:3000 grafana/grafana',
    docs: 'https://grafana.com/docs/',
    use: 'Подключается к Prometheus, Loki, ClickHouse, PostgreSQL и др.' },
  { id: 3, name: 'Zabbix', cat: 'monitoring', icon: '👁️', color: '#d40000',
    ...
    use: 'Универсален для сетевого оборудования и серверов.' },
  { id: 4, name: 'Netdata', cat: 'monitoring', icon: '⚡', color: '#00ab44',
    desc: 'Real-time мониторинг с нулевой конфигурацией. 2000+ метрик из коробки.',
    tags: ['real-time', 'лёгкий', 'агент'],
    install: 'bash <(curl -Ss https://my-netdata.io/kickstart.sh)',
    docs: 'https://learn.netdata.cloud/',
    use: 'Быстрый старт. Хорош для разовой диагностики.' },
  { id: 5, name: 'Loki', cat: 'monitoring', icon: '📜', color: '#ff9900',
    ...
    use: 'Пара к Prometheus. Promtail собирает логи, Loki хранит.' },
  // Сеть
  { id: 6, name: 'Wireshark', cat: 'network', icon: '🦈', color: '#1a74c0',
    desc: 'Анализатор сетевых пакетов. GUI и CLI (tshark).',
    tags: ['перехват', 'анализ', 'pcap'],
    install: 'apt install wireshark',
    docs: 'https://www.wireshark.org/docs/',
    use: 'Захват трафика, анализ протоколов, отладка.' },
  { id: 7, name: 'nmap', cat: 'network', icon: '🗺️', color: '#00b4d8',
    desc: 'Сетевой сканер. Обнаружение хостов, открытых портов, OS fingerprinting.',
  ...
  { id: 9, name: 'WireGuard', cat: 'network', icon: '🔒', color: '#88171a',
    desc: 'Современный быстрый VPN-протокол, встроен в ядро Linux.',
    tags: ['VPN', 'быстрый', 'ядро'],
    install: 'apt install wireguard',
    docs: 'https://www.wireguard.com/',
    use: 'Значительно проще и быстрее OpenVPN. Рекомендован для новых деплоев.' },
  // Безопасность
  { id: 10, name: 'fail2ban', cat: 'security', icon: '🛡️', color: '#e63946',
    desc: 'Блокировка IP при подозрительной активности на основе лог-файлов.',
    tags: ['bruteforce', 'SSH', 'iptables'],
    install: 'apt install fail2ban',
    docs: 'https://www.fail2ban.org/wiki/',
    use: 'Автоматически блокирует IP после N неудачных попыток.' },
  { id: 11, name: 'Lynis', cat: 'security', icon: '🔎', color: '#f4a261',
    desc: 'Аудит безопасности и hardening-рекомендации для Linux.',
  ...
  { id: 12, name: 'ClamAV', cat: 'security', icon: '🦠', color: '#e76f51',
    ...
    use: 'clamscan -r /home — рекурсивное сканирование.' },
  // Автоматизация
  { id: 13, name: 'Ansible', cat: 'automation', icon: '🤖', color: '#e00',
    desc: 'Agentless инфраструктура как код. Playbooks на YAML.',
    tags: ['IaC', 'agentless', 'YAML'],
    install: 'pip install ansible',
    docs: 'https://docs.ansible.com/',
    use: 'ansible-playbook site.yml -i inventory — запуск плейбука.' },
  { id: 14, name: 'Terraform', cat: 'automation', icon: '🏗️', color: '#7b42bc',
    desc: 'Провижонинг облачной инфраструктуры декларативным способом.',
    tags: ['IaC', 'cloud', 'plan/apply'],
    install: 'apt install terraform',
    docs: 'https://developer.hashicorp.com/terraform/docs',
    use: 'terraform init → plan → apply → destroy' },
  { id: 15, name: 'Jenkins', cat: 'automation', icon: '⚙️', color: '#d33833',
    desc: 'CI/CD сервер с огромной экосистемой плагинов.',
  ...
  { id: 16, name: 'Docker', cat: 'containers', icon: '🐳', color: '#2496ed',
    desc: 'Платформа контейнеризации. Основной инструмент DevOps.',
  ...
  { id: 17, name: 'Kubernetes', cat: 'containers', icon: '☸️', color: '#326ce5',
    ...
    use: 'kubectl get pods -A — посмотреть все поды во всех namespace.' },
  ...
  { id: 20, name: 'Redis Insight', cat: 'database', icon: '🔴', color: '#dc382d',
    desc: 'GUI для Redis: просмотр данных, профилирование запросов.',
  ...
  { id: 22, name: 'Restic', cat: 'storage', icon: '💾', color: '#4a90d9',
    desc: 'Быстрый инкрементальный бэкап с шифрованием.',
...
export default function Tools() {
  ...
  return (
    <div>
      <h1 className="page-title">Инструменты <span className="accent">sysadmin</span></h1>
