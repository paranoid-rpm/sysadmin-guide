...
const PORTS = [
  ...
  { port: 2379, proto: 'TCP', name: 'etcd', cat: 'Контейнеры', risk: 'high', desc: 'etcd клиентский API (Kubernetes). Только внутри кластера' },
...
export default function Ports() {
  ...
  return (
    <div>
      ...
      <div className="ports-count">{filtered.length} портов</div>
          ...
          <tbody>
            {filtered.map(p => {
              ...
              return (
                <tr
                  key={p.port}
                  className={`ports-row ${selected === p.port ? 'sel' : ''}`}
                  onClick={() => setSelected(selected === p.port ? null : p.port)}
                >
                  ...
                  <td>
                    <span className="port-risk" style={{ '--rc': ri.color }}>{ri.label}</span>
                  </td>
