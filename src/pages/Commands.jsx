...
const SECTIONS = [
  {
    ...
    commands: [
      ...
      { cmd: 'df -hT', desc: 'Использование дисков с типом файловой системы.' },
  ...
  },
  {
    ...
    commands: [
      ...
      { cmd: 'pgrep -u www-data nginx', desc: 'Поиск PID процесса по имени и пользователю.' },
      { cmd: 'lsof -p PID', desc: 'Открытые файлы процесса.' },
      { cmd: 'strace -p PID', desc: 'Системные вызовы процесса.' },
      { cmd: 'nice -n 10 command', desc: 'Запуск с пониженным приоритетом.' },
      { cmd: 'nohup command &', desc: 'Запуск в фоне с игнорированием SIGHUP.' },
  ...
  },
  {
    ...
    commands: [
      ...
      { cmd: 'ss -s', desc: 'Сводка по сетевым соединениям.' },
  ...
  },
  {
    ...
    commands: [
      ...
      { cmd: 'systemctl enable --now nginx', desc: 'Включить автозапуск и сразу запустить.' },
      { cmd: 'systemctl list-units --failed', desc: 'Список упавших юнитов.' },
  ...
  },
  {
    ...
    commands: [
      ...
      { cmd: 'docker exec -it container bash', desc: 'Зайти внутрь контейнера.' },
      ...
      { cmd: 'docker compose up -d --build', desc: 'Пересобрать и запустить стек.' },
      { cmd: 'docker compose down -v', desc: 'Остановить стек и удалить volumes.' },
      { cmd: 'docker system prune -af', desc: 'Очистить всё неиспользуемое.' },
      { cmd: 'docker image ls --filter dangling=true', desc: 'Найти «висячие» образы.' },
  ...
  },
  {
    ...
    commands: [
      ...
      { cmd: 'zcat /var/log/syslog.*.gz | grep ERROR', desc: 'Поиск ошибок в сжатых логах.' },
      { cmd: 'dmesg -T | tail -50', desc: 'Последние записи ядра с метками времени.' },
      ...
      { cmd: 'logrotate -d /etc/logrotate.conf', desc: 'Проверить конфиг logrotate без применения.' },
      { cmd: 'multitail /var/log/nginx/access.log /var/log/syslog', desc: 'Просмотр нескольких логов в одном окне.' },
    ]
  },
  {
    id: 'postgres',
    ...
    commands: [
      ...
      { cmd: 'pg_dump -Fc mydb > mydb.dump', desc: 'Бэкап базы в формате custom.' },
      { cmd: 'pg_restore -d mydb mydb.dump', desc: 'Восстановление из custom‑дампа.' },
      { cmd: 'vacuumdb -a -z', desc: 'VACUUM ANALYZE всех баз.' },
      { cmd: 'psql -U postgres -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname=\'mydb\' AND pid<>pg_backend_pid();"', desc: 'Сбросить все соединения к базе.' },
  ...
  },
  {
    ...
    commands: [
      ...
      { cmd: 'openssl s_client -connect host:443 2>/dev/null | openssl x509 -noout -dates', desc: 'Срок действия SSL‑сертификата удалённого хоста.' },
  ...
  },
  {
    ...
    commands: [
      ...
      { cmd: 'git stash list', desc: 'Список сохранённых изменений (stash).' },
      { cmd: 'git stash pop', desc: 'Восстановить последний stash.' },
      { cmd: 'git reset --hard origin/main', desc: 'Сбросить ветку до состояния удалённой.' },
      { cmd: 'git rebase -i HEAD~3', desc: 'Интерактивный rebase последних трёх коммитов.' },
      { cmd: 'git cherry-pick <sha>', desc: 'Перенести конкретный коммит в текущую ветку.' },
      { cmd: 'git bisect start && git bisect bad && git bisect good <sha>', desc: 'Бинарный поиск коммита, сломавшего код.' },
  ...
  },
  {
    ...
    commands: [
      ...
      { cmd: 'nginx -T | grep server_name', desc: 'Показать все server_name из конфигурации.' },
      ...
      { cmd: 'awk \'{sum+=$10} END{print sum/NR" bytes avg"}\' /var/log/nginx/access.log', desc: 'Средний размер ответа в байтах.' },
      { cmd: 'grep " 5[0-9][0-9] " /var/log/nginx/access.log | tail -20', desc: 'Последние 5xx‑ошибки.' },
      { cmd: 'certbot renew --dry-run', desc: 'Проверить автообновление SSL‑сертификатов без применения.' },
  ...
  },
  {
    ...
    commands: [
      ...
      { cmd: 'sed -i \'s/old/new/g\' file.conf', desc: 'Заменить все вхождения строки в файле.' },
      ...
      { cmd: 'grep -v "^#" /etc/nginx/nginx.conf | grep -v "^$"', desc: 'Конфиг Nginx без комментариев и пустых строк.' },
      { cmd: 'grep -oP \'\\d{1,3}(\\.\\d{1,3}){3}\' access.log | sort -u', desc: 'Уникальные IP‑адреса из лога (regex).' },
...
export default function Commands() {
  ...
  return (
    <div>
      ...
      <p className="page-subtitle">{totalAll} команд по {SECTIONS.length} категориям. Клик по команде — скопировать в буфер.</p>
